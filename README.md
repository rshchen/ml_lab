...
│   └── openml/                 # SciKit-Learn 自動下載的 MNIST 原始數據快取
├── src/ml_lab/                 # 核心公共底層函數庫 (高內聚模組)
│   ├── __init__.py
...
* **`visualize_gradient_field.py`**：利用 `meshgrid`、`contour` 與 `quiver` 繪製二維權重底面的等高線與反梯度方向箭頭場，引導反思「處處垂直等高線」與「萬流歸宗」的凸優化幾何本質。
